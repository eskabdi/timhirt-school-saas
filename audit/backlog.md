...
| WP-02 review AC-7 | Error contract across allow-listed RPCs is still mixed: `acknowledge_alert` on a foreign or missing id returns success and changes nothing; other RPCs raise P0001 with distinct messages. Settle one scheme (42501 denied, P0002 not found) in the API contract. | WP-12 |
| WP-02 review AC-6 | `create_*_job` has no idempotency key, so a double submit creates two jobs. | WP-12 |
| WP-02 review AC-9 | The RPC inventory covers definer functions only; 9 invoker RPCs (e.g. `enroll_admission_application`, `promote_students_batch`) are still executable by anon (RLS applies). Inventory every PostgREST-exposed function and revoke anon from the app-only ones. | WP-06 / WP-18 |
| WP-02 review AC-11 | No OpenAPI spec for the RPCs; classification lives in `definer_inventory.md`. | WP-18 |
| WP-02 review AZ-8 | Writing definer RPCs bypass RLS, so WP-06/WP-07's module, impersonation read-only and aal2 policies will not cover them; `auto_assign_exam_seats` has no gradebook module check. Add the checks inside each writing definer RPC. | WP-06 / WP-07 |
| WP-02 review SEC-7 / TI-8 / AC-14 | `has_module`'s caller-tenant guard adds lookups per row in 57 module-gate policies (about +27–40% on a 5–20k-row list). Move the gates to an initplan form (`(select …)`) or a tenant-less `has_module(p_module)`. | WP-06 (performance-reviewer) |
| WP-02 review TI-3 | Cross-tenant probes exist for most allow-listed uuid functions but not all (`get_class_rank`, `get_student_grade_history`, `dashboard_overview`, `is_guardian_of`, `is_teacher_of_class` were probed by the reviewer, not committed). Add them, and a catalog rule that every allow-listed function with a uuid argument has a probe. | WP-06 |
| WP-02 review AZ-6 | Whether the `rls-tests` CI job is a required status check on the protected branches is outside the repo (OWNER_ACTIONS C5). | Owner |
| WP-09 | `expire_approvals()` has no scheduler; expired requests are refused at decision time but stay `pending` in the table until swept. Schedule it (pg_cron or a cron Edge Function). | WP-16 |
