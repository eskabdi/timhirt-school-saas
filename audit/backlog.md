...
| WP-00 r3 RG3-3 | No Deno test covers the https guard inside `record-fee-payment` / `verify-admission-bank-url`. The DB CHECK (pgTAP) and `httpsHref` (Vitest) back it up. Make both handlers injectable, as `manage-integration-credentials/handler.ts` is, and test that `http:` returns `https_required` with no insert. | WP-03 (both functions are rewritten there) |
