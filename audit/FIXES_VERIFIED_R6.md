...
**Branch / PR:** `claude/timhirt-security-audit-kan0ei` → `fix/production-readiness-r6`, after WP-01 (PR #9).

### Findings

| ID | Status | Evidence |
|---|---|---|
| H-01: 42 anon-executable SECURITY DEFINER functions, several trusting caller-supplied tenant/user ids | **fixed** (repo; deploy pending) | Migration `20260926000001_r6_definer_lockdown.sql`. `catalog_definer_security.sql` is now hard: grants equal `definer_allowlist.sql` exactly; anon can execute exactly one definer function (`get_security_settings`, password keys only). `definer_lockdown.sql` 28/28, including the Report 3 Appendix A-1 probes (anon `get_email_for_user`, `create_health_alert` into tenant B, `complete_job`, `get_config`, `cleanup_old_audit_logs`; tenant-A admin `create_export_job(B)`, `has_module(B)`). |
| L-07: cross-tenant oracles (`has_module`, `get_config`) | **fixed** | `has_module` answers only for the caller's tenant; `get_config`/`is_feature_enabled` are service_role-only. |
| 13 definer functions without `search_path`, 39 without `pg_temp` (WP-01 AZ-1) | **fixed** | Every definer pins `public, pg_temp`; the guard requires pg_temp last. |
| 11 tables without FORCE RLS (WP-01 baseline) | **fixed** | `catalog_rls_coverage.sql` hard: RLS enabled and forced on every table. Production owner postgres has BYPASSRLS (read-only check), so migrations and cron are unaffected. |

### Implementation (plan WP-02) and recon adjustments

| Plan item | Done | Notes |
|---|---|---|
| 1. Inventory | Yes | `supabase/security/definer_inventory.md`, generated by `scripts/db/definer-inventory.py`: 1 Public, 23 Private, 23 Internal, 18 Trigger-only. |
| 2. Allow-list | Yes | `supabase/security/definer_allowlist.sql` (28 grants, each with a reason), mirrored by the migration's GRANTs and enforced in both directions by the catalog guard. |
| 3. Migration | Yes, **adjusted** | Loop revokes PUBLIC/anon/authenticated and pins search_path on every definer; explicit re-grants. The plan's `alter default privileges … revoke … from public` was dropped: a schema-scoped revoke cannot remove PostgreSQL's global PUBLIC default, and a global one would also close future extension functions. CI is the control (a new definer fails until it is allow-listed or revoked). Found because the global form persisted across harness runs and broke pgTAP itself; the shim now restores the built-in default at the start of every run. |
| 4. Derive tenant internally | Yes, **adjusted** | Signatures kept (57 module-gate and 700+ helper call sites in policies unchanged); the functions now ignore or refuse an id that is not the caller's. Trust is decided by the invoking role (`current_setting('role')` is `authenticated`/`anon` for end users even inside a definer), not by a missing JWT: the first version used `auth.uid() is null` and two existing suites (custom_role_enforcement #8, resource_permissions #20) caught that it misclassified a postgres session with a leftover JWT claim. |
| 5. `get_config()` split | **Adjusted** | Nothing calls `get_config` (app or Edge); it is service_role-only instead of split. Revisit if a client needs public config. |

### Gate (local)

- pgTAP: 110 migrations, 63/63 suites; remaining TODOs are WP-05 (storage) and WP-06 (module gate) only.
- Proof the tests fail without the fix: on a database at the WP-01 head, `definer_lockdown.sql` fails 17 assertions and the hard catalog guards fail.
