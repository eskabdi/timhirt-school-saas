# Timhirt Audit, Report 3: Consolidated Security, Architecture & INSA Compliance Audit

> **Report 3 of 3: Consolidated audit.**
> Merges the baseline audit (Report 1: `audit-2026-09-24-report-1-baseline.md`, 32 findings) with the INSA assessment (Report 2: `audit-2026-09-24-report-2-insa.md`). This is the authoritative current view: 36 findings.
> Changes from Report 1: added H-07, M-14, M-15, M-16 and L-13; M-15 replaces L-11; M-08 extended; §2.4 INSA scorecard and §3a finding-to-INSA mapping added.
...
> **INSA baseline (revised 2026-09-24).** The first version of this report had no INSA checklist and used OWASP WSTG / ASVS L2 as a stand-in. The owner has since supplied **"INSA Technical Development Enforcer"**, a summary of INSA's *Web Application Security Testing Requirements* §4.2.1–4.2.5 and §5 (APIs), organised as Phases 1–6. **§2.4 now scores the project against it phase by phase**, and the findings it produced (H-07, M-14–M-16, L-13) are in §3. The WSTG table (§2.3) stays as supplementary test coverage.
>
> **Caveat:** the supplied document summarises INSA requirements. It does not reproduce the official text, and it does not say which Phase corresponds to which §4.2.x clause. §2.4 is therefore keyed to its Phase numbering. Before formal submission, map it clause by clause to the official INSA document.
>
> Separately, the blueprint cites §21.9 for INSA reasoning, and that section does not exist (the document ends at §20).
...
| H-07 | **The INSA documentation set describes controls that don't exist.** The blueprint's Architecture, DFD, Stack Inventory and Security Functionality sections claim Stripe, SendGrid, Upstash, Sentry, a SIEM drain, HMAC-verified webhooks, Dependabot + `npm audit` in CI, pinned Edge Function dependencies, MFA, a 30-minute idle timeout / 12-hour cap, and OpenAPI exports. None of these are in the code. Submitted as-is, the documentation would misstate the system to the INSA assessor. | High (compliance) |
...
**Findings by severity: 1 Critical · 7 High · 16 Medium · 12 Low = 36.** The first version had 32. The INSA re-mapping added H-07, M-14, M-15, M-16 and L-13, and promoted L-11 to M-15.

**INSA readiness verdict: NOT READY for submission.** Scores per §2.4: Phase 1 Partial · Phase 2 Fail · Phase 3 Partial · Phase 4 Fail · Phase 5 Fail · Phase 6 Partial.
...
### 2.3 OWASP WSTG / ASVS L2 categories (supplementary test coverage)
...
| Session Management | Partial | 1 h JWT with refresh rotation and reuse detection. Idle logout is client-side only. Impersonation sessions are not revoked (M-04). Tokens are in Web Storage and the idle timeout is 60 min, client-side only (M-15). |
...
### 2.4 INSA Technical Development Requirements, phase by phase

Keyed to the supplied *INSA Technical Development Enforcer* (§4.2.1–4.2.5, §5). "Evidence" points at this repo; "Blueprint" is `docs/school-saas-architecture-blueprint.md`.
...
| L-10 | Repository documentation drift | Low | ISO A.5.37 | `CLAUDE.md` ("38 migrations + 5 pgTAP suites"; actual 105 / 55); blueprint cites §21.9 (absent) | Developer docs lag the code. The larger INSA-documentation problem is H-07. | Repo read | Refresh `CLAUDE.md` counts. Add §21 or remove the citations. |
...
## 3a. Findings Mapped to INSA Phases
...
8. **H-07** Before any INSA submission, regenerate the INSA documentation set (DFD L0–L2, architecture, ERD, Actor Inventory, Stack Inventory, Security Functionality Document) from the code as it is after the P0/P1 fixes. Nothing should be claimed that isn't implemented.
...
13. **M-14 / M-15 / M-16** OpenAPI spec + sample payloads for all Edge Functions. Server-side 30 min inactivity / 12 h timebox in GoTrue. Configure a WAF and a SIEM log drain with alerting. These are INSA Phase 5 / 3 / 1.2 blockers.

### P3 — Low (backlog / hardening)
14. L-01 Tayitu/Jiret everywhere · L-02 FORCE RLS on 11 tables · L-03 CORS allow-list · L-04 sanitise the editor · L-05 no SVG in the public bucket · L-06 fixed `APP_URL` · L-07 (closed by H-01) · L-08 SAST, secret scanning, SHA-pinned actions · L-09 jsonb snake_case · L-10 refresh CLAUDE.md · L-12 escape filter terms · L-13 fail-closed staging seed guard + complete audit accounts.
