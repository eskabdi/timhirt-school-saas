...
- The existing gates: typecheck, lint, unit tests (Vitest), i18n, locale parity, build, and pgTAP (64 suites).
...
- EXECUTE is closed by default: only `service_role` (Edge Functions) and the owner can call a definer function unless `supabase/security/definer_allowlist.sql` grants it, with a reason. Today: 24 functions for `authenticated` (9 RLS helpers the policies call, 15 RPCs the app calls), 1 for `anon` (`get_security_settings`, which returns only the password policy to anon, so the invite page can show it), and 3 helpers for the view owner `timhirt_view_owner`. Trigger functions need no grant.
- Functions never trust a caller-supplied identity. `get_email_for_user` returns only the caller's own email; `get_role_for_user` and `get_tenant_id_for_user` answer only for the caller or a user in the caller's tenant; `has_resource_permission` only for the caller; `has_module` only for the caller's tenant (super_admin: any); `create_export_job`/`create_import_job` only in the caller's tenant and only for a school_admin; `acknowledge_alert` only the caller's tenant's alerts, only for a school_admin. Service contexts (service_role, cron, migrations) are recognised by the invoking role (`current_setting('role')`), not by a missing JWT.
- The job, health, cleanup, config and feature-flag functions are service_role-only.
- FORCE ROW LEVEL SECURITY is now on every table in `public` (the last 11 were closed; their owner, postgres, has BYPASSRLS, so migrations and cron are unaffected).
- Control in CI: `catalog_definer_security.sql` (grants equal the allow-list exactly; search_path with pg_temp; no definer outside public), `catalog_rls_coverage.sql` (RLS enabled and forced everywhere), and `definer_lockdown.sql` (28 real calls as anon, a cross-tenant admin, a student and service_role, including the Report 3 Appendix A-1 probes).

**API inventory → RPC classification.** `supabase/security/definer_inventory.md` (generated by `scripts/db/definer-inventory.py`): 1 Public (anon), 23 Private (authenticated), 23 Internal (service_role), 18 Trigger-only, each with its callers (policies, triggers, app, Edge Functions).

**Residual:** PostgreSQL still grants EXECUTE to PUBLIC on *new* functions by default (changing the global default for postgres would also close future extension functions). A new definer function therefore starts open, and CI fails it until its migration revokes and re-grants per the allow-list.
