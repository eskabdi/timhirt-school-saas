...
- ~~O-02~~ **answered 2026-09-24:** the domain has mail (hostns.io; mailboxes superadmin@, noreply@, info@). The owner supplied SPF, DKIM, DMARC and PTR. Live: only DMARC; **MX, SPF and DKIM are missing**, and the MX value is still needed. The session token can't write Vercel DNS, so the owner must add them. See `docs/runbooks/domain-edux-et.md`.
