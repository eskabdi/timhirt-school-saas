...
- No online payment gateway. This version takes manual bank-transfer payments
  only (fix plan WP-03). The Telebirr H5 C2B gateway was decommissioned in
  R6 WP-00 (C-01); Chapa and Stripe were canceled earlier.
...
# Apply all 17 migrations (core -> academic -> attendance/fees -> HR/payroll ->
# RLS -> storage -> extended modules -> extended RLS -> extended storage ->
# security hardening -> base table grants -> RLS recursion fix ->
# column-level grants -> integration credentials -> settlement enum-cast fix ->
# module permission matrix -> registration stepper fields)
supabase db push

# Secrets for Edge Functions (never commit these). SMS provider keys are
# entered by super_admin at /platform/integrations and stored encrypted in
# Supabase Vault. No payment-gateway secrets exist in this version.
...
supabase functions delete telebirr-notify
