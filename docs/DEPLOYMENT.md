...
- Telebirr H5 C2B merchant credentials (fabric app key/secret, appid, merch
  code — testbed available without a production merchant account) — **not
  required before first deploy**; configure them afterward via
  `/platform/integrations` as super_admin. `process-fee-payment` returns a
  clear "not configured yet" error rather than failing until that's done.
  Chapa and Stripe are canceled — neither is wired into this codebase.
...
# 011: Telebirr's fabric_app_secret and signing private key are normally
# entered/generated by super_admin at /platform/integrations, stored
# encrypted in Supabase Vault. There is no env-var fallback for the RSA
# private key (telebirr-generate-keypair writes it directly to Vault, never
# through this shell). Chapa/Stripe are canceled — nothing to set for them.
