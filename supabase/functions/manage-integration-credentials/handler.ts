// Request handler for manage-integration-credentials, separated from
// Deno.serve (index.ts) so tests can inject the auth and rate-limit
// dependencies and a fake database (R6 WP-00 review R2-TV-2).
import { errors, json, corsHeaders, type AuthContext } from "../_shared/security.ts";
import { Payload } from "./schema.ts";
import { PROVIDER_CONFIG_KEYS, PROVIDER_SECRET_KEYS, sameKeys } from "./keys.ts";

export type HandlerDeps = {
  requireRole: (req: Request, allowed: string[]) => Promise<AuthContext | Response>;
  rateLimit: (key: string, limit: number, windowMs: number) => Promise<boolean>;
};

export function makeHandler({ requireRole, rateLimit }: HandlerDeps) {
