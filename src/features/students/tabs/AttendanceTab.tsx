    ...
    <div className="space-y-4">
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-2">
          <div className="mb-2 flex items-center justify-between">
            <div>
              <h2 className="font-display text-lg font-bold text-ink">Yearly Trend | ዓመታዊ አዝማሚያ</h2>
              <p className="text-xs text-ink-faint">Academic Year</p>
            </div>
            <div className="flex gap-3 text-xs text-ink-soft">
              <span className="flex items-center gap-1"><i className="h-3 w-3 rounded-sm bg-ok" />Present</span>
              <span className="flex items-center gap-1"><i className="h-3 w-3 rounded-sm bg-danger" />Absent</span>
              <span className="flex items-center gap-1"><i className="h-3 w-3 rounded-sm bg-warn" />Holiday</span>
            </div>
          </div>
          {byMonth.size ? (
              ...
              {[...byMonth.entries()].map(([month, days]) => (
                <div key={month}>
                  ...
                  <div className="grid grid-cols-6 gap-1">
                    {days.map((d, i) => <i key={i} title={`Day ${d.day}: ${d.status}`} className={`h-4 w-4 rounded-sm ${cellColor(d.status)}`} />)}
                  </div>
                </div>
              ))}
            </div>
          ) : <p className="py-8 text-center text-ink-faint">No attendance records.</p>}
        </Card>

        {/* Absence insights */}
        <Card>
          <h2 className="font-display text-lg font-bold text-ink">Absence Insights | ግንዛቤ መግለጫ</h2>
          <div className="mt-3 space-y-3">
            <div>
              <div className="flex items-center justify-between text-sm"><Badge tone="ok">EXCUSED</Badge><span className="text-ink-faint">{insights.excused} Absences</span></div>
            ...
            <div>
              <div className="flex items-center justify-between text-sm"><Badge tone="danger">UNEXCUSED</Badge><span className="text-ink-faint">{insights.unexcused} Absence</span></div>
      ...
      <Card>
        <div className="mb-3 flex items-center justify-between">
          <div>
            <h2 className="font-display text-lg font-bold text-ink">Attendance Log | የክትትል መዝገብ</h2>
            <p className="text-xs text-ink-faint">Complete history</p>
          </div>
        </div>
        <table className="w-full text-sm">
          <thead className="bg-sidebar text-left text-xs uppercase text-ink-faint">
            <tr><th className="px-4 py-2">Date (GC)</th><th className="px-4 py-2">Date (EC)</th><th className="px-4 py-2">Status</th><th className="px-4 py-2">Reason/Note</th><th className="px-4 py-2">Marked By</th><th className="px-4 py-2">Action</th></tr>
          </thead>
          <tbody className="divide-y divide-line">
            {paged.length ? paged.map((r, i) => (
              <tr key={i} className={r.status === "absent" ? "bg-danger-tint/40" : ""}>
                ...
                <td className="px-4 py-3"><Badge tone={STATUS_TONE[r.status] ?? "neutral"}>{r.status === "late" ? "Tardy" : r.status}</Badge></td>
                <td className="px-4 py-3 text-ink-soft">{r.reason || "-"}</td>
                <td className="px-4 py-3 text-ink-soft">{markers?.get(r.recorded_by) ?? "—"}</td>
                <td className="px-4 py-3"><button className="text-navy hover:underline">View Detail</button></td>
              </tr>
            )) : <tr><td colSpan={6} className="py-10 text-center text-ink-faint">No attendance records.</td></tr>}
          </tbody>
        </table>
        {(rows?.length ?? 0) > PAGE && (
          <div className="mt-3 flex items-center justify-between text-sm text-ink-faint">
            <span>Showing {(page - 1) * PAGE + 1}-{Math.min(page * PAGE, rows!.length)} of {rows!.length} entries</span>
            <div className="flex items-center gap-1">
              <button disabled={page === 1} onClick={() => setPage(page - 1)} className="rounded border border-line px-2 py-1 disabled:opacity-40">‹</button>
              <span className="rounded bg-navy px-3 py-1 text-white">{page}</span>
              <button disabled={page === totalPages} onClick={() => setPage(page + 1)} className="rounded border border-line px-2 py-1 disabled:opacity-40">›</button>
            </div>
          </div>
        )}
      </Card>
