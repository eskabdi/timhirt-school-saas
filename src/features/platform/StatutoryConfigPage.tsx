...
export function StatutoryConfigPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="font-display text-2xl font-bold">Statutory configuration</h1>
      <Card>
        <h2 className="mb-3 font-semibold">Income tax brackets (Proc. 979/2016 Art. 11, as amended by 1395/2025)</h2>
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-line">
            {brackets?.map((b) => (
              <tr key={b.id}>
                <td className="py-1.5">{b.income_from} ETB</td>
                <td className="py-1.5">{b.income_to ?? "∞"} ETB</td>
                <td className="py-1.5">{b.rate_pct}%</td>
                <td className="py-1.5">{b.deduction_amount} ETB</td>
      ...
      <Card>
        <h2 className="mb-3 font-semibold">Pension rates (Proc. 715/2011)</h2>
        {pension?.map((p) => (
          <p key={p.id} className="text-sm">Employee {p.employee_pct}% · Employer {p.employer_pct}% — effective <EthDate value={p.effective_from} /></p>
        ))}
      </Card>
      <p className="text-xs text-warn">⚠️ Verify against the proclamation currently in force before go-live (§18.3).</p>
    </div>
  );
}
